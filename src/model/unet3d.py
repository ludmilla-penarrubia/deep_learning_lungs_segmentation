...
# 3DUnet implemntation of : https://github.com/UdonDa/3D-UNet-PyTorch/blob/master/src/model.py


class UNet3D(nn.Module):
    def __init__(self, n_channels, n_classes, num_filters):
        super().__init__()
        ...
        # Down sampling
        self.down_1 = conv_block_2_3d(self.n_channels, self.num_filters, activation)
        ...
        # Output
        self.out = conv_block_3d(self.num_filters, n_classes, activation)
    ...
    def forward(self, x):
        ...
        out_smax = self.softmax(out)  # -> [1, 3, 128, 128, 128] (unchanged)
        return out_smax
...
# show input shape
#print(summary(UNet3D(1,2,4), torch.zeros((1, 1, 128, 128, 128)), show_input=True))

# # show output shape
# print(summary(UNet3D(1,2,4), torch.zeros((1, 1, 128, 128, 128)), show_input=False))

# show output shape and hierarchical view of net
#print(summary(UNet3D(1,2,4), torch.zeros((1, 1, 128, 128, 128)), show_input=False, show_hierarchical=True))